 ...
 *
 * Remplace le stub `/api/rgpd/delete` (jamais implémenté, gardé derrière `ENABLE_ACCOUNT_DELETION`
 * sans jamais rien écrire en base) : ici la demande est réellement enregistrée et effectivement
 * purgée par le CRON `/api/cron/account-deletion`.
 */
...
export async function POST(req: NextRequest) {
  ...
  const { allowed } = await rateLimit(`account-delete:${user.id}`, 5, 60);
  if (!allowed) {
...
export async function DELETE(_req: NextRequest) {
  ...
  const { allowed } = await rateLimit(`account-delete-cancel:${user.id}`, 5, 60);
  if (!allowed) {
    return NextResponse.json({ error: "Trop de demandes, réessaie dans une minute." }, { status: 429 });
  }
